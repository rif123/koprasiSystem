...
<ul class="breadcrumb">
	...
	<li class="divider"></li>
	<li>Edit Item Promo</li>
</ul>
<div class="innerLR">

<form class="form-horizontal" id="form_input" method="POST" action="<?=site_url('promo/update/content_promo/'.$promo->idx_promo)?>" enctype="multipart/form-data">
	<!-- Widget -->
        <div class="widget widget-gray widget-body-gray">
                <!-- Widget heading -->
                <div class="widget-head"><h4 class="heading">Edit Item Promo</h4></div>
                <!-- // Widget heading END -->
                <div class="widget-body">
					<div class="row">
                        ...
                        <!-- Column -->
                                <div class="col-md-6" style="border: 1px solid #bbb;padding: 10px 10px;float:none;width:100%;margin-bottom: 4px;">
									...
									<?php if(($promo->idx_type_promo<>2)&&($promo->idx_type_promo<>5)){
										...
										<!-- Group -->
											<div class="form-group">
													...
													<div class="col-md-8"><input class="form-control" value="'.@$content_promo->row()->discount.'" id="discount" name="discount" style="width:20%" type="text"></div>
											</div>
										<!-- // Group END -->';
									}
									?>
									<?php if($promo->idx_type_promo==4){
										echo'
											...
											<div class="form-group">
													...
													<div class="col-md-8"><input class="form-control" value="'.$content_promo->row()->minimum_transaksi.'" id="minimum_transaksi" style="width:40%" name="minimum_transaksi" type="text"></div>
											</div>
											<!-- // Group END -->';
									}
									?>
									<?php if($promo->idx_type_promo==5){ ?>
									 <p style="padding: 10px 0px;font-weight: bold;border-bottom: 1px solid #bbb;">Pilih kota yang gratis biaya pengiriman</p>
										<script>
											var i=<?=($free_shipping_city->num_rows()+1)?>;
										...
										<!-- Group -->
											<div class="form-group">
													...
													<div class="col-md-8" id="box_kota">
														<?php
															$n=1;
															foreach($free_shipping_city->result() as $wcf){
																echo"<div class='k' id='k".$n."'><img onClick=Remove('k".$n."') src='".base_url()."assets/images/remove.png'/><input type='text'  class='form-control' name='kota[]' value='".$wcf->kota."'/></div>";
															$n++;
															}
														?>
													</div>
								 ...
								 <div class="col-md-6" style="border: 1px solid #bbb;padding: 10px 10px;float:none;width:100%">
									<script>
										$(document).ready(function() {
												...
												"bServerSide": true,
												"iDisplayLength" :500,
												"sAjaxSource": "<?=site_url('promo/list_product_use')?>?idx_promo=<?=$promo->idx_promo?>"
												} );
										});
									</script>
									 <p style="padding: 10px 0px;font-weight: bold;border-bottom: 1px solid #bbb;">Produk yg terdiskon</p>
									<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper form-horizontal" style="margin: 0 2em;" role="grid">
										<div class="row">
											<table class="dataTables_wrapper form-horizontal" role="grid" id="use" >
												<thead>
													<tr>
								...
								<? } ?>
								<?php if(($promo->idx_type_promo<>4)&&($promo->idx_type_promo<>2)){ ?>
								 <p style="padding: 10px 0px;font-weight: bold;border-bottom: 1px solid #bbb;">Tambah Produk Promo</p>
								 <div class="col-md-6" style="border: 1px solid #bbb;padding: 10px 10px;float:none;width:100%">
								 ...
								 <div class="col-md-6" style="border: 1px solid #bbb;padding: 10px 10px;float:none;width:100%">
									<script>
										$(document).ready(function() {
												...
												"bServerSide": true,
												"iDisplayLength" :500,
												"sAjaxSource": "<?=site_url('promo/list_product_use_bogof')?>?idx_promo=<?=$promo->idx_promo?>"
												} );
										});
									</script>
									 <p style="padding: 10px 0px;font-weight: bold;border-bottom: 1px solid #bbb;">Produk yg terdiskon</p>
						...
						<?php if($promo->idx_type_promo<>4){ ?>
                                ...
                                <button type="submit" onClick="$('#fl_submit').val(1)" name="save" value="save" class="btn btn-icon btn-primary glyphicons circle_ok"><i></i>Update Produk</button>
						<?php } ?>
								<button type="submit" name="save" value="finish" class="btn btn-icon btn-primary glyphicons circle_ok"><i></i>Update & Selesai</button>
                                <button type="button" onClick="window.location='<?=site_url("promo/edit/promo/".$promo->idx_promo)?>'" class="btn btn-icon btn-default glyphicons circle_remove"><i></i>Kembali</button>
                        </div>
                        <!-- // Form actions END -->
