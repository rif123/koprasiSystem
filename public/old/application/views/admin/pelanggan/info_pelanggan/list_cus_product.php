...
<div class="section_container" style="border-top: 1px solid #C2BEBE;"> 
    ...
    <section> 
      <!--CART STARTS-->
      <div id="shopping_cart" class="full_pelanggan">
        <div class="cart_table">
          <table class="data-table cart-table" id="shopping-cart-table" cellpadding="0" cellspacing="0">
			...
			foreach($list_product->result() as $item){
				$bonus="";
				if($pelanggan->cek_bogof($item->idx_product)->num_rows()>0){
					$bonus="<b style='color:green'>Bonus ".$pelanggan->cek_bogof($item->idx_product)->row()->bogof_caption."</b>";
				}
				echo'
				<tr>
				  ...
				  <td class="align_right vline"><span class="price">Rp.'.number_format($item->harga_discount*$item->qty,0,',','.').'</span></td>
				</tr>';
			}
			?>
				<tr>
				  <td style='text-align: right;vertical-align: middle;' colspan="6"><span class="price">Subtotal</span></td>
				  <td class="align_center vline" colspan="2"><span class="price">Rp. <?=number_format($order->total_tagihan_produk,0,',','.')?></span></td>
				</tr>
          </tbody>
		 </table>
		 <?php
		  ...
		  <div class="totals">
							<table id="totals-table" style="width:55%">
								...
								<tr class="expedisi_area">
									...
									<td class="align_left" width="55%"><?=$pelanggan->convert_expedisi($order->jne_expedisi)?></td>
								</tr>
								<tr>
