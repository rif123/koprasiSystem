...
<meta name="description" content="<?=$identitas->description?>,<?=$Cquery->tag_desc?>"/>
<meta name="author" content="<?=$identitas->site_title?>">
<meta property="og:url" content="<?='http://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']?>">
<meta name="keywords" content="<?=$identitas->keyword?> <?=$Cquery->tag_key?>" />
<meta property="og:title" content="<?=$site_title?>">
<meta property="og:description" content="<?=$identitas->description?>,<?=$Cquery->tag_desc?>">
            ...
            <div id="side_nav">
				<div class="sideNavCategories">
					...
					<ul class="category departments">
						<li class="header"><?=$category->get_nm($idx_path)?></li>
						<?php
						foreach($category_product->result() as $c){
							if($c->parent<>0){
								$Qc=$this->additional_model->get_category($c->idx_category_product);
								if($Qc->num_rows()==0){
									if($category->cek_parent($c->idx_category_product)>0){
									...
									echo'<li class="menu_cont"><a href="'.site_url('category/'.$c->idx_category_product.'/'.strtolower(str_replace(" ","-",$c->nm_categrory_product))).'" title="'.$c->nm_categrory_product.'">'.$c->nm_categrory_product.'</a>
											<ul class="side_sub_menu">';
												foreach($Qc->result() as $sw){
													echo'<li> <a href="'.site_url('category/'.$sw->idx_category_product.'/'.strtolower(str_replace(" ","-",$c->nm_categrory_product)).'/'.strtolower(str_replace(" ","-",$sw->nm_categrory_product))).'" title="'.$sw->nm_categrory_product.'">'.$sw->nm_categrory_product.'</a></li>';		
												}
										echo'</ul>
										</li>';
            ...
            <div id="main_content">
               <!-- <div class="category_banner"> <img src="Leisure%20Magento%20Theme_productlist_files/promo_cat_banner.jpg"> </div>-->
				<!--breadcrumb-->
				 <ul class="breadcrumb">
				 <?php
					$parent	=$category->parent_product($Cquery->idx_category_product);
					foreach($parent->result() as $wcp){
						echo'<li><a href="#" style="cursor: default;">'.$category->get_nm($wcp->idx_path).'</a></li>';
					}
					echo'<li class="active"><a href="#" style="cursor: default;">'.$category->get_nm($Cquery->idx_category_product).'</a></li>';
				 ?>
				 </ul>
				 <!--/breadcrumb-->
				<?php if($list_product->num_rows()>0){	?>
						 ...
						 <?php
							...
							if($list_product->num_rows()>0){
								foreach($list_product->result() as $w){
									...
									$discount		='';
									$sold_out		='';
									if($category->check_promo($w->idx_product)->num_rows()>0)
									{
										$pr=$category->check_promo($w->idx_product)->row();
									...
									if($category->sold_out($w->idx_product,$w->type_product)){
										$sold_out		='<span class="sold_out"><span>SOLD OUT</span></span>';
									}
										echo'
											<li jcarouselindex="'.$n.'" style="float: left; list-style: none outside none;" class="jcarousel-item jcarousel-item-horizontal jcarousel-item-1 jcarousel-item-1-horizontal"> 
												...
												'.$discount.$sold_out.'
													...
													<div class="pr_attr'.$n.'">';
													$Qattr=$category->get_attribute_group($w->idx_product);
													echo'
													...
													if($category->free_ongkir($w->idx_product)>0){
