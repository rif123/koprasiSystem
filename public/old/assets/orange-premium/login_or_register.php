...
<link rel="stylesheet" href="<?=$assets.$template?>/css/form_style.css"><!-- mobile setting -->
...
<div class="wrapper">
 ...
 <div class="section_container" style="border-top: 1px solid #C2BEBE;">
        ...
        <section>
            <div class="full_page">
                <h4>Sign Up atau login untuk menyelesaikan belanja. </h4>
				<div class="col-left">
					<h3>Sudah Pernah Berbelanja?</h3>
					<form class="form1" id="frm_register" action="<?=site_url('page/login_proses/order')?>" method="POST">
				...
				<div class="col-right">
					<h3>Baru Pertama Kali Belanja?</h3>
					<div id='bx-next'><button onClick="window.location='<?=site_url('checkout')?>'" class='sc-button small orange'>Daftar Dan Lanjutkan</button></div>
				</div>
            </div>
        </section>
        <!--Mid Section Ends-->
...
<script>
	$(document).ready(function(){
		...
		$('#ajax-status').ajaxStart(function(){
			$('#message').hide();
			$(this).fadeIn('fast');
			/*$(this).html("<img src='<?=$assets."demo/images/loading.gif"?>'/>");*/
		});
