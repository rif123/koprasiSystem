...
class Setting extends Controller
{
    ...
    {
        $data = ST::select('setting_name', 'value')->whereIn('setting_name', array("site_title","site_description","meta_description","meta_keyword","site_logo", "favicon"))->get()->toArray();
        $b= array();
        ...
        if ($validator->fails()) {
			...
			if (\Input::hasFile('favicon')) {
                if (!empty(\Input::file('favicon')) ) {
                    $imageNameFavicon = \Input::file('favicon')->getClientOriginalName();
                    \Input::file("favicon")->move(public_path().'/images/', $imageName);
                }
            }
            if (!empty($imageNameFavicon)) {
                $affected = \DB::update("update settings set
                    `value` = CASE setting_name WHEN 'site_title'
                    THEN ? WHEN 'site_description'
                    THEN ?  WHEN 'meta_description'
                    THEN ?  WHEN 'meta_keyword'
                    THEN ?  WHEN 'site_logo'
                    THEN ?  WHEN 'favicon'
                    THEN ? END
                    where setting_name
                    IN('site_title','site_description','meta_description','meta_keyword','site_logo', 'favicon')",
                    [$title,$description,$metadescription,$metakeyword,$imageName,$imageNameFavicon]);
            } else {
                $affected = \DB::update("update settings set
                    `value` = CASE setting_name WHEN 'site_title'
                    THEN ? WHEN 'site_description'
                    ...
                    THEN ? END
                    where setting_name
                    IN('site_title','site_description','meta_description','meta_keyword','site_logo')",
                    [$title,$description,$metadescription,$metakeyword,$imageName]);
            }
