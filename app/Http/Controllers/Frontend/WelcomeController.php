...
use App\Models\JenisUsaha as JS;
...
class WelcomeController extends Controller
{
    ...
    {
        // if ($all) {
        //     $data['project'] = JS::with('project_category')->get();
        // } else {
        ...
        //     }
        // }
        $project = JS::getPhotoProd();
        $project_category = JS::all();
        $groupProject = [];
        foreach ($project as $k => $v) {
            $groupProject[$v->jenis_usaha][] = ['pasPhotoProduk' => $v->pasPhotoProduk, 'jenis_usaha' => $v->jenis_usaha, 'brand_usaha' => $v->brand_usaha, 'kd_jenis_usaha' => $v->kd_jenis_usaha];
        }
        $data['project'] = $groupProject;
        $allPc = [];
        foreach ($project_category as $key => $value) {
            $allPc[] = $value->nama_jenis_usaha;
        }
        $data['project_category'] = $allPc;
        return $data;
    }
    ...
    public function index()
    {
        ...
        foreach ($data['widget'] as $a) {
            ...
            } elseif ($a['name'] == 'testimonial_section') {
                $data['testi']= Testi::where('active', 1)->get();
            } elseif ($a['name'] == 'process_section') {
                $data['process_title']= PC::select('title')->get();
                $data['process_textbox']= PC::select('textbox')->get();
            } elseif ($a['name'] == 'process_section') {
                $data['process_title']= PC::select('title')->get();
            } elseif ($a['name'] == 'skill_section') {
                $data['skill'] = Skill::all();
            } elseif (($a['name'] == 'news_section') || ($a['name'] == 'latest_news_footer')) {
                $data['post'] = $this->postData();
            } elseif (($a['name'] == 'contact_section') || ($a['name'] == 'contact_us_footer')) {
    ...
    public function projectShow($id)
    {
        ...
        $list = JS::getPhotoProd($id);
        $data['data'] = !empty($list[0]) ? $list[0] : "";
        // print_R($data);die;
        return view("frontend.project-show", $data)->with('parser', $this->parser);
    }
